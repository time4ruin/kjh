...
def replace_nop_repeat_in_region(n_value: int, file_path: Path = BRANCHES_FILE) -> None:
    ...
    /* PAD2 start */ and /* PAD2 end */ in branches.c.
    If n_value > MAX_NOP_PER_BLOCK, split into multiple NOP_REPEAT blocks.
    """
    content = file_path.read_text()

    # regex to find region (keep the markers)
    pattern = re.compile(r'(/\*\s*PAD2 start\s*\*/)(.*?)(/\*\s*PAD2 end\s*\*/)', flags=re.DOTALL)
    m = pattern.search(content)
    if not m:
        raise RuntimeError("Could not find '/* PAD2 start */ ... /* PAD2 end */' region in branches.c")
