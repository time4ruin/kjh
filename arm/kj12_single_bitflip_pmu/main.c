...
int tests()
{
    ...
    for (int i = 0; i < testsize; i++)
    {
        ...
        // set_ghr();
        miss[i] = branch1(0);
        // printf("[MISS] PMU Counter: %lu\n", miss[i]);
    }

    print_histogram(hit, testsize, "HIT");
    print_histogram(miss, testsize, "MISS");
    double mean_h = trimmed_mean(hit, 100, 0);
    double mean_m = trimmed_mean(miss, 100, 0);
    printf("average (hit): %.2f\n", mean_h);
    printf("average (miss): %.2f\n", mean_m);
    // double stddev_h = trimmed_stddev(hit, 100, 20);
    // double stddev_m = trimmed_stddev(miss, 100, 20);
    // printf("average (hit): %.2f (%.2f)\n", mean_h, stddev_h);
    // printf("average (miss): %.2f (%.2f)\n", mean_m, stddev_m);
    free(hit);
    free(miss);
    return 0;
}
