...
# ---------- Step1: branch1의 target_bit를 0으로 만들기 (측정→보정 루프) ----------
def solve_branch1_bit_zero(target_bit: int,
                           pad1_init: int = 0,
                           padfine1_init: int = 0
                           ) -> Tuple[int, int, str]:
    """
    branch1 주소의 target_bit를 0으로 만들 때까지,
    실측 b1을 보고 필요한 NOP(4B 단위)를 계산해 PAD1/PADFINE1을 증가시키며 정렬.
    - padfine1은 최대 PADFINE1_MAX까지만, 초과분은 pad1로 흡수
    - pad1/padfine1은 줄이지 않고 '앞으로만' 이동
    """
    if target_bit < 2:
    ...
    for _ in range(MAX_ITER):
        ...
        if (b1i & BIT_MASK) == 0:
            # 정렬 완료
            print(f"[i] bit{target_bit}: branch1 aligned at {b1} "
                  f"(PAD1={pad1}, F1={padfine1})")
            return pad1, padfine1, b1

        # 다음 2^bit 경계까지 필요한 바이트/ NOP
        rem = b1i & (MOD - 1)
        add_bytes = MOD - rem          # 0 < add_bytes <= MOD
        add_nops = (add_bytes + 3) // 4

        # 먼저 padfine1(≤MAX)로 흡수, 나머지는 pad1로
    ...
    raise RuntimeError(f"Alignment failed for bit {target_bit}: not converged within {MAX_ITER} iterations.")
