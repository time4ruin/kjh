...
# This script runs locally only — no adb/remote device support
...
# NOP instruction size in bytes on the current architecture (x86: 1 byte per `nop`)
NOP_BYTE_SIZE      = 1
...
def push_bin(bin_name: str) -> None:
    ...
    # local execution only — nothing to push
    print(f"[+] local mode: verified {bin_name} exists")
...
def kill_remote_by_name(bin_name: str) -> None:
    ...
    """
    try:
        # kill local processes matching the binary name
        subprocess.run(["pkill", "-f", bin_name], check=False)
...
def run_on_device_and_grab_addrs(bin_name: str,
    ...
    """
    Run local `taskset` ./bin_name and parse stdout for outer/inner addresses.
    - who: "VIC" or "ATK"
    - when both addresses are obtained, pkill the process and return stdout
    """
    cmd = ["taskset", CPU_MASK, f"./{bin_name}"]
...
def align_outer(target_outer: int,
    ...
    for _ in range(ALIGN_MAX_ITER):
        ...
        # number of NOPs to add (round up by NOP_BYTE_SIZE)
        add_nops = (delta_bytes + (NOP_BYTE_SIZE - 1)) // NOP_BYTE_SIZE
...
def run_attacker_victim_parallel():
    ...
    # attacker/victim run locally with taskset
    atk_cmd = ["taskset", CPU_MASK, "./attacker.bin"]
    vic_cmd = ["taskset", CPU_MASK, "./victim.bin"]
    ...
    # 결과 파일 are produced locally (p1.txt / p2.txt)
    print("[+] result files: p1.txt, p2.txt (local)")
