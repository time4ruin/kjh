...
def insert_record(b1_addr: str, b2_addr: str, da_val: str, avg_hit: float, avg_miss: float, db_path: Path = DB_FILE):
    ...
    cur.execute(
        "INSERT INTO res (b1a, b2a, da, hit, miss, arch) VALUES (?, ?, ?, ?, ?, ?)",
        (b1_int, b2_int, da_int, avg_hit, avg_miss, ARCHITECTURE+"-"+CPUNUM)
    )
    conn.commit()
    conn.close()
...
def run_experiment_and_store(n_value: int) -> Tuple[str, str, str, Optional[float], Optional[float]]:
    ...
    print(f"[+] Inserted: b1={b1}, b2={b2}, da={da}, hit={avg_hit}, miss={avg_miss}, arch={ARCHITECTURE}-{CPUNUM}")
